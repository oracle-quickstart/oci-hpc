...
def check_gpu_count():
    ...
    try:
        ...
        if shape in ["BM.GPU.L40S-NC.4", "BM.GPU.A10.4", "BM.GPU.GB200.4", "BM.GPU.GB200-v2.4", "BM.GPU.GB300.4"]:
            expected_gpus = 4
...
####################################################
    ...
    if (run_all or args.oca_stat) and (not shape in ["BM.GPU.GB200.4", "BM.GPU.GB200-v2.4", "BM.GPU.GB300.4", "BM.GPU.L40S.4", "BM.GPU.A10.4"]):
        try:
    ...
    if (run_all or args.gpu_pcie) and ( not shape in ["BM.GPU.GB200.4","BM.GPU.GB200-v2.4","BM.GPU.GB300.4"]):    
        try:
    ...
    if (run_all or args.ip_address) and ( not shape in ["BM.GPU.GB200.4","BM.GPU.GB200-v2.4","BM.GPU.GB300.4"]):  
        if oca_state == "COMPLETED":
            try:
