...
Script Summary:
===========================================================================================
This script performs a series of health checks on an OCI (Oracle Cloud Infrastructure) 
host to ensure the proper functioning of GPU-based instances. It supports both bare-metal 
(BM) and virtual machine (VM) shapes, skipping certain checks for VM shapes where hardware 
...
===========================================================================================
1. **Oracle Cloud Agent Version Check**:
   - Ensures the Oracle Cloud Agent is installed and up-to-date (minimum version: 1.39.0).

2. **RTTCC (Round Trip Time Congestion Control) Status Check**:
   - Verifies RTTCC is disabled for supported GPU shapes.

3. **ECC (Error-Correcting Code) Errors Check**:
   - Checks for SRAM and DRAM ECC errors on NVIDIA or AMD GPUs.

4. **Row Remap Errors Check**:
   - Detects row remap errors on GPUs and recommends actions based on severity.

5. **GPU Count Check**:
   - Validates the number of GPUs available on the system matches the expected count.

6. **GPU PCIe Link Width Check**:
   - Ensures the PCIe link width for GPUs is as expected (e.g., x16).

7. **GPU Bandwidth Test**:
   - Measures GPU bandwidth using the `BandwidthTest` class (only runs if explicitly requested).

8. **Bus Status Check**:
   - Checks if any devices have fallen off the PCIe bus.

9. **RDMA Link Status Check**:
   - Verifies the status of RDMA (Remote Direct Memory Access) links for Mellanox devices.

10. **RDMA Link Flapping Check**:
    - Detects RDMA link flapping issues over a specified time interval.

11. **GPU Xid Errors Check**:
    - Checks for GPU Xid errors, which indicate hardware or driver issues.

12. **WPA Authentication Check**:
    - Verifies WPA authentication status for RDMA interfaces (skipped for VM shapes).

13. **Fabric Manager Status Check**:
    - Ensures the NVIDIA Fabric Manager service is running for supported GPU shapes.

14. **CPU Performance Profile Check**:
    - Confirms that all online CPUs are set to the "performance" profile.

15. **Pending Bad Pages Check (AMD GPUs)**:
    - Checks for pending bad pages on AMD GPUs.
...
# 1.1 Check if the Oracle Cloud Agent is installed and up-to-date
def get_oca_version():
...
# 2.1 Check RTTCC status for supported GPU shapes and return status log.
def check_rttcc_status():
    devices = get_devices()
    if not devices:
        #logger.info(f"RTTCC check not required for shape: {metadata['shape']}")
        pass
        return []
...
# 3.1 Check ECC errors for NVIDIA or AMD GPUs.
...
# 4.1 Check for row remap errors on GPUs.
...
# 5.1 Check the number of GPUs available on the system.
def check_gpu_count():
...
# 6.1 Checks PCIe link width for NVIDIA or AMD based on instance shape.
def check_gpu_pcie():
    metadata = get_metadata()
    shape = metadata.get('shape', '')
    ...
    if shape == "BM.GPU.MI300X.8":
        try:
            ...
            if result.returncode != 0 or "No devices were found" in output:
                logger.warning("GPU PCIe Width Test: Failed - No devices were found")
                return ["No GPUs detected"]
...
# 7.1 GPU Bandwidth test using the BandwidthTest class

# 8.1 Check to see if any devices have fallen of the bus
def check_bus():
    command = ['lspci', '-v']
...
# 9.1 Check RDMA link status for Mellanox devices.
...
# 10.1 RDMA link flapping test using the LinkFlappingTest class from external file

# 11.1 Xid error check using the XidChecker class from external file

# 12.1 Determine the shape and required authenticated count for WPA authentication check.
...
# 13.1 Check the status of the Fabric Manager
...
# 14.1 Retrieve online CPUs and check if their profile is set to 'performance'.
...
# 15.1 Check for pending bad pages on GPUs, for AMD only.
def check_bad_pages():
    try:
...
# =============================================== Section 2: Main function and args to run all checks (1.2 - 15.2)===============================================
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ocaver', action='store_true', help='Run OCA version check')
    parser.add_argument('--rttcc', action='store_true', help='Run RTTCC status check')
    parser.add_argument('--eccerr', action='store_true', help='Run ECC errors check')
    parser.add_argument('--rowerr', action='store_true', help='Run row remap errors check')
    parser.add_argument('--gpucount', action='store_true', help='Run GPU count check')
    parser.add_argument('--gpupcie', action='store_true', help='Run GPU PCIe check')
    ...
    parser.add_argument('--busstat', action='store_true', help='Run bus status check')
    parser.add_argument('--rdmalink', action='store_true', help='Run RDMA link status check')
    parser.add_argument('--rdmalinkflap', action='store_true', help='Run RDMA link flapping check')
    ...
    parser.add_argument('--xiderror', action='store_true', help='Run GPU Xid errors check')
...
# =============================================== Section 3: Function calls to run all health checks ===============================================
    ...
    # 4.3 Check for row remap errors
    ...
    # 8.3 Check for devices fallen off the bus
    ...
    # 10.3 Check RDMA link flapping
    if run_all or args.rdmalinkflap:
        try:
    ...
    # 11.3 Check GPU Xid errors
    if run_all or args.xiderror: 
        try:
            xc = XidChecker()
    ...
    # 12.3 Check WPA Authentication status
    ...
    # 13.3 Check Fabric Manager status
    ...
    # 14.3 Check if CPU profile is performance
    ...
    # 15.3 Check if AMD GPU has pending bad pages
...
#======================================================= Section 4: Summarize the results and recommend actions ========================================================
    ...
    # 1.4 Summarize OCA version check
    if run_all or args.ocaver:
    ...
    # 2.4 Summarize RTTCC status check
    if run_all or args.rttcc:
        if shape != "BM.GPU.MI300X.8":
    ...
    # 3.4 Summarize ECC errors check
    if run_all or args.eccerr:
        if len(ecc_issues) > 0:
            ecc_error = False
    ...
    # 4.4 Summarize row remap errors check
    if run_all or args.rowerr:
    ...
    # 6.4 Summarize GPU PCIe width check
    if run_all or args.gpupcie:
        if gpu_pcie_results:
    ...
    # 7.4 Summarize GPU bandwidth test
    ...
    # 8.4 Summarize bus status check
    if run_all or args.busstat:
        if bus_results:
    ...
    # 9.4 Summarize RDMA link status check
    if run_all or args.rdmalink:
    ...
    # 10.4 Summarize RDMA link flapping check
    if run_all or args.rdmalinkflap:
    ...
    # 11.4 Summarize GPU Xid errors check
    if run_all or args.xiderror:
    ...
    # 12.4 Summarize WPA Authentication check
    if run_all or args.wpa_auth:
        if wpa_auth_results:
    ...
    # 13.4 Summarize Fabric Manager check
    if run_all or args.fabric_mgr:
    ...
    # 14.4 Summarize CPU profile check
    if run_all or args.cpu_profile:
    ...
    # 15.4 Summarize pending bad pages check for AMD
    if run_all or args.bad_page:
        if bad_page_issues:
