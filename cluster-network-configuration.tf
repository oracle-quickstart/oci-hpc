resource "oci_core_instance_configuration" "cluster-network-instance_configuration" {
  count          = var.rdma_enabled && (!var.stand_alone || var.cluster_network_shape == "BM.GPU.GB200.4" || var.cluster_network_shape == "BM.GPU.GB200-v2.4" || var.cluster_network_shape == "BM.GPU.GB300.4") ? 1 : 0
  ...
  instance_details {
    ...
    launch_details {
      ...
      freeform_tags = {
        ...
        "memory_cluster_name" = var.cluster_network_shape == "BM.GPU.GB200.4" || var.cluster_network_shape == "BM.GPU.GB200-v2.4" || var.cluster_network_shape == "BM.GPU.GB300.4" ? "${local.cluster_name}-fabric1" : ""
      }
      metadata = {
