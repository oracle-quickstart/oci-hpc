...
def run_add(nodes, count, names):
    ...
    for first_node in nodes:
        if first_node.shape in ["BM.GPU.GB200.4","BM.GPU.GB200-v2.4","BM.GPU.GB300.4"]:
...
def run_add_memory_fabric( nodes, count, fabric_id , gpu_memory_cluster_name, instancetype=None):
    ...
    if fabric_id is None:
        logger.error(f"For BM.GPU.GB200.4 or BM.GPU.GB200-v2.4 or BM.GPU.GB300.4, the memory fabric needs to be specified, Exiting")
        sys.exit(1)
    first_node=nodes[0]
...
def getLaunchInstanceDetailsFromInstanceType(config, controller_hostname, cn_ocid, cluster_name, hostname=None, memory_cluster_name=None):
    ...
    try:
        ...
        if shape in ["BM.GPU.GB200.4","BM.GPU.GB200-v2.4","BM.GPU.GB300.4"]:
...
def create_cluster(config, count, cluster_name, controller_hostname, names, gpu_memory_fabric=None, gpu_memory_cluster_name=None):
    ...
    if not config.stand_alone or config.shape in ["BM.GPU.GB200.4","BM.GPU.GB200-v2.4","BM.GPU.GB300.4"]:
        ...
        if config.rdma_enabled:
            if config.shape in ["BM.GPU.GB200.4","BM.GPU.GB200-v2.4","BM.GPU.GB300.4"]:
...
def delete_cluster(cluster_name,nodes_list):
    ...
    if cluster_type == "CN":
        ...
        logger.error("A BM.GPU.GB200.4, BM.GPU.GB200-v2.4, or BM.GPU.GB300.4 instance should not be calling this function")
