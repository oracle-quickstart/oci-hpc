...
if [ ! -d "${UV_INSTALL_DIR}" ]; then
  ...
  sudo mkdir -p "${UV_LOCAL_PYTHON_INSTALL_DIR}" "${UV_PYTHON_INSTALL_DIR}"
  if [ "$ID" = "ubuntu" ] ; then
...
if [ "$ID" = "ol" ] || [ "$ID" = "centos" ] ; then
  if [ "$vid" = "7" ]; then
    ...
    sudo def install -y python3 python3-pip python3-dnf java-11-openjdk-headless http-parser
  fi
  ...
  sudo systemctl stop apt-daily-upgrade.timer apt-daily.timer 2>/dev/null || true
  sudo systemctl disable apt-daily-upgrade.timer apt-daily.timer 2>/dev/null || true
  sudo systemctl mask apt-daily-upgrade.service apt-daily.service 2>/dev/null || true

  sleep 10s

  sudo apt-mark hold linux-oracle linux-headers-oracle linux-image-oracle 2>/dev/null || true
  ...
  if [ "$ID" = "ubuntu" ] && [ "$VERSION_ID" = "20.04" ] ; then
    ...
    while [ "$apt_success" -ge 1 ]; do
      echo "Installing base packages (python3, pip, jq, openjdk, http-parser)..."
      sleep 10s
      sudo apt-get -y install python3 python3-pip jq openjdk-11-jre-headless libhttp-parser2.9
      apt_success=$?
      echo "apt-get exit: $apt_success"
    done
  fi
  if [ "$ID" = "ubuntu" ] && [ "$VERSION_ID" = "20.04" ] ; then
    fix_apt
...
# install uv
if [ -n "${creating_python_install_dir:-}" ]; then
...
# Generate a fresh ansible.cfg from ansible-config
"${VENV_PATH}/bin/ansible-config" init --disabled -t all | sudo tee /etc/ansible/ansible.cfg >/dev/null

# For OL8 only, comment out ALL remote_tmp lines within [powershell]
# to avoid "option 'remote_tmp' already exists".
if [[ "$ID" = "ol" && "$vid" -eq 8 ]]; then
  sudo sed -i '/^\[powershell\]/,/^\[/ s/^remote_tmp/#remote_tmp/' /etc/ansible/ansible.cfg
fi
...
# Force remote_tmp under [defaults] (and only there)
...
# Ensure [callback_default] section exists and uses YAML output
