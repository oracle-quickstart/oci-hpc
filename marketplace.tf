...
//  listing_number = split(".", var.marketplace_listing)[0]
    ...
    mp_login_listing_id = var.use_standard_image_login ? var.use_old_marketplace_image ? var.old_marketplace_listing_id :var.marketplace_listing_id_HPC : ""
    ...
    mp_login_version_id = var.use_old_marketplace_image ? var.marketplace_version_id[split(".", var.old_marketplace_listing)[0]] : var.marketplace_version_id["HPC_OL7"]
}
...
data "oci_core_app_catalog_listing_resource_versions" "login_app_catalog_listing_resource_versions" {
    count = var.use_standard_image_login ? 1 : 0
