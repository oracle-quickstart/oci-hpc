...
# EXEC=all_gather_perf sbatch -N8 ./nccl_run_alltoall.sbatch

if [[ -z "${EXEC}" ]]; then
  export EXEC_CMD="/opt/oci-hpc/nccl-test/build/alltoall_perf"
else
...
do
  ...
  then
    ...
    var_NCCL_TOPO_FILE="/opt/oci-hpc/healthchecks/B300MP.xml"
  ...
  NCCL_TOPO_ARGS=()
  if [ -n "${var_NCCL_TOPO_FILE}" ]; then
    NCCL_TOPO_ARGS=(-x NCCL_TOPO_FILE="${var_NCCL_TOPO_FILE}")
  fi
